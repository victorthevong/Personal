...
//    System.out.println("FInished Entry adds");
  ...
  public void testBPlusTreeRandomInsertSortedScanInnerSplit() {
    ...
    public void test1RandomLookupCall() {
        Random rand = new Random(0); //const seed
        HashSet set = new HashSet();
        for (int i = 0; i < 5*intLeafPageSize; i++) {
            int val = rand.nextInt();
            set.add(val);
            bp.insertKey(new IntDataType(val), new RecordID(val, i));
        }
        Iterator iter = set.iterator();

        while (iter.hasNext()) {
            int x = (Integer) iter.next();
            Iterator<RecordID> rids = bp.lookupKey(new IntDataType(x));
            assertTrue(rids.hasNext());
        }
    }
    ...
    @Test
    @Category(StudentTest.class)
    public void test2Insert50LeafPages() {
        for (int i = 0; i < 50*intLeafPageSize; i++) {
            bp.insertKey(new IntDataType(i), new RecordID(i,0));
        }
        ...
        assertEquals(50*intLeafPageSize, counter);
    }

    @Test
    @Category(StudentTest.class)
    public void test3InsertFrontandBack() {
    ...
    @Test
    @Category(StudentTest.class)
    public void test4PInsertForBackUp() {
        for (int i = 0; i < 20; i ++) {
            for (int j = 0; j < 10; j ++) {
                bp.insertKey(new IntDataType(i), new RecordID(i, j));
            }
        ...
        while (rids.hasNext()) {
            RecordID rid = rids.next();
            assertEquals(5 + counter/10, rid.getPageNum());
            assertEquals(counter % 10, rid.getSlotNumber());
            counter++;
        }
        assertEquals(counter, 310);

    }

    @Test
    @Category(StudentTest.class)
    public void test5InsertIterateFrom() {
        for (int i = 300; i >= 0; i--) {
            bp.insertKey(new IntDataType(i), new RecordID(i,0));
        }
        Iterator<RecordID> rids = bp.sortedScanFrom(new IntDataType(10));
