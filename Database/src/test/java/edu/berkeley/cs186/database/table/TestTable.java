...
public class TestTable {
  ...
  public void testTableIteratorGapBackDurable() throws DatabaseException {
    ...
    for (int i = 0; i < 500; i++) {
      assertTrue(iRec.hasNext());
      input.getValues().get(1).setInt(i);
//      System.out.println(input  + " " + iRec.next());
      assertEquals(input, iRec.next());
    }
    assertFalse(iRec.hasNext());
  ...
  public void testTableIteratorGapFrontDurable() throws DatabaseException {
    ...
    @Test
    @Category(StudentTest.class)
    public void test1Interleaving() throws DatabaseException {
        Record input = TestUtils.createRecordWithAllTypes();
        RecordID[] recordIDs = new RecordID[5000];
        int numEntriesPerPage = table.getNumEntriesPerPage();
        for (int i = 0; i < numEntriesPerPage; i++) {
            recordIDs[i] = table.addRecord(input.getValues());
            assertEquals(1, recordIDs[i].getPageNum());
            assertEquals(i, recordIDs[i].getSlotNumber());
        }

        for (int i = 0; i < numEntriesPerPage; i++) {
            recordIDs[i+numEntriesPerPage]= table.addRecord(input.getValues());
            assertEquals(2, recordIDs[i+numEntriesPerPage].getPageNum());
            assertEquals(i, recordIDs[i+numEntriesPerPage].getSlotNumber());
        }
        ...
        for (int i = 0; i < numEntriesPerPage; i++) {
            recordIDs[i+2*numEntriesPerPage] = table.addRecord(input.getValues());
            assertEquals(3, recordIDs[i+2*numEntriesPerPage].getPageNum());
            assertEquals(i, recordIDs[i+2*numEntriesPerPage].getSlotNumber());
        }
    ...
    @Test
    @Category(StudentTest.class)
    public void test2InnerGaps() throws DatabaseException {
        Record input = TestUtils.createRecordWithAllTypes();
        RecordID[] recordIds = new RecordID[5000];
    ...
    @Test
    @Category(StudentTest.class)
    public void test3DeleteNothing() throws DatabaseException {
        Record input = TestUtils.createRecordWithAllTypes();
        RecordID id = table.addRecord(input.getValues());
    ...
    @Test
    @Category(StudentTest.class)
    public void test4Insert10Records() throws DatabaseException {
        Record input = TestUtils.createRecordWithAllTypes();
        RecordID[] recordIDs = new RecordID[10*table.getNumEntriesPerPage()];
        for (int i = 0; i < 10*table.getNumEntriesPerPage(); i++) {
            recordIDs[i] = table.addRecord(input.getValues());
            assertEquals(i/table.getNumEntriesPerPage()+1, recordIDs[i].getPageNum());
            assertEquals(i%table.getNumEntriesPerPage(), recordIDs[i].getSlotNumber());
        }

    }
    @Test
    @Category(StudentTest.class)
    public void test5UpdateNonexistentRecord() throws DatabaseException {
        Record input = TestUtils.createRecordWithAllTypes();
        try {
